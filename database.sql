...
BEGIN
    CREATE TABLE DANH_MUC_TRO_CHOI (
...
BEGIN
    CREATE TABLE DANH_MUC_VI_TRI (
        ...
        CONSTRAINT FK_VITRI_GAME  FOREIGN KEY (ma_tro_choi) REFERENCES DANH_MUC_TRO_CHOI(ma_tro_choi)
    );
END
GO

IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Arena of Valor')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'Arena of Valor', 'MOBA');
IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'League of Legends')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'League of Legends', 'MOBA');
IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Free Fire')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'Free Fire', 'BATTLEROYALE');
IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'PUBG')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'PUBG', 'BATTLEROYALE');
IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Valorant')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'Valorant', 'FPS');
IF NOT EXISTS (SELECT 1 FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'CS:GO')
    INSERT INTO DANH_MUC_TRO_CHOI (ten_game, the_loai) VALUES (N'CS:GO', 'FPS');
GO

DECLARE @AOV INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Arena of Valor');
DECLARE @LOL INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'League of Legends');
DECLARE @FREEFIRE INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Free Fire');
DECLARE @PUBG INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'PUBG');
DECLARE @VALORANT INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'Valorant');
DECLARE @CSGO INT = (SELECT ma_tro_choi FROM DANH_MUC_TRO_CHOI WHERE ten_game = N'CS:GO');
...
BEGIN
    CREATE TABLE HO_SO_IN_GAME (
        ...
        CONSTRAINT FK_HSG_TC       FOREIGN KEY (ma_tro_choi)         REFERENCES DANH_MUC_TRO_CHOI(ma_tro_choi),
...
-- ---------------------------------------------------------------
-- 5.2.1  TRONG_TAI_GIAI_DAU  (Referee assignments)
-- ---------------------------------------------------------------
...
BEGIN
    CREATE TABLE GIAI_DOAN (
        ...
        the_thuc              NVARCHAR(50)  NOT NULL,
        thu_tu                INT           NOT NULL,        so_doi_di_tiep        INT           NOT NULL CONSTRAINT DF_GDO_DOIDITIEP DEFAULT 0,
...
BEGIN
    CREATE TABLE TRAN_DAU (
        ...
        CONSTRAINT CHK_TD_TRANGTHAI CHECK (trang_thai IN ('chua_dau','san_sang','dang_dau','cho_ket_qua','da_hoan_thanh','huy_bo','bye')),
...
SELECT
    ...
    (SELECT COUNT(1) FROM DANH_MUC_TRO_CHOI       WHERE is_active   = 1)                                         AS tong_game_active;
GO
...
BEGIN
    CREATE TABLE DANG_KY_TRONG_TAI (
        ...
        CONSTRAINT FK_DangKyTrongTai_TroChoi FOREIGN KEY (ma_tro_choi) REFERENCES DANH_MUC_TRO_CHOI(ma_tro_choi)
    );
END
GO
